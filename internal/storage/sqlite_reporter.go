...
type resultReporter struct {
	storage       *feishusdk.ResultStorage
	db            *sql.DB
	table         string
...
func newResultReporter(storage *feishusdk.ResultStorage) (*resultReporter, error) {
	if storage == nil {
		return nil, nil
	}
...
func (r *resultReporter) flushOnce() {
	...
	for _, row := range rows {
		...
		log.Trace().
			...
			Msg("result reporter dispatching capture row to feishusdk")
...
func (r *resultReporter) dispatchRow(row pendingResultRow) error {
	if r.storage == nil {
		return pkgerrors.New("storage: feishusdk reporter nil")
	}
...
func (r *resultReporter) dispatchBatch(rows []pendingResultRow) error {
	...
	records := make([]feishusdk.ResultRecordInput, 0, len(rows))
	for _, row := range rows {
...
func (r pendingResultRow) toResultRecord() feishusdk.ResultRecordInput {
	...
	return feishusdk.ResultRecordInput{
		DatetimeRaw:         dtRaw,
...
func parsePollInterval() time.Duration {
	...
	dur := env.Duration(envResultReportPollInterval, fallback)
	if dur <= 0 {
		return fallback
	}
	return dur
...
func parseBatchSize() int {
	const fallback = 10
	size := env.Int(envResultReportBatchSize, fallback)
	if size <= 0 {
		return fallback
	}
	return size
...
func parseReportTimeout() time.Duration {
	...
	dur := env.Duration(envResultReportHTTPTimeout, fallback)
	if dur <= 0 {
		return fallback
	}
	return dur
