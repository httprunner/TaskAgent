...
func resolveResultTableName() string {
	if val := env.String("RESULT_SQLITE_TABLE", ""); val != "" {
		return val
	}
...
func NewManager(cfg Config) (*Manager, error) {
	...
	if feishuStorage != nil {
		...
		names = append(names, "feishusdk-reporter")
		manager.name = strings.Join(names, ",")
	}
	return manager, nil
}

func getOrStartGlobalReporter(storage *feishusdk.ResultStorage) (*resultReporter, error) {
...
func buildSinks(cfg Config) ([]Sink, *feishusdk.ResultStorage, error) {
	sinks := make([]Sink, 0, 3)
	...
	var feishuStorage *feishusdk.ResultStorage
	if enableFeishu {
		var err error
...
func shouldEnableJSONL(cfg Config) bool {
	...
	if strings.TrimSpace(env.String(envDisableJSONL, "")) != "" {
		return false
	}
	return true
}
...
func shouldEnableFeishu(cfg Config) bool {
	...
	val := strings.TrimSpace(env.String(envEnableFeishu, ""))
...
func newFeishuStorage() (*feishusdk.ResultStorage, error) {
	storage, err := feishusdk.NewResultStorageFromEnv()
	if err != nil {
		return nil, err
	}
	return storage, nil
...
func EnsureResultReporter() error {
	if strings.TrimSpace(env.String(feishusdk.EnvResultBitableURL, "")) == "" {
	...
	if storage == nil {
		log.Warn().Msg("storage: feishusdk storage factory returned nil, reporter not started")
		return nil
	}
...
func (m *Manager) Close() error {
	...
	if m.reporter != nil {
		if err := m.reporter.Close(); err != nil {
			errs = append(errs, pkgerrors.Wrap(err, "feishusdk reporter close failed"))
		}
	}
	if m.reader != nil {
...
func resolveDatabasePath() (string, error) {
	if custom := strings.TrimSpace(env.String(envTrackingDBPath, "")); custom != "" {
