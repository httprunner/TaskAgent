...
func TestEnsureResultReporterSkipsWithoutEnv(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		os.Unsetenv(feishusdk.EnvResultBitableURL)
	})

	os.Unsetenv(feishusdk.EnvResultBitableURL)
	called := 0
	feishuStorageFactory = func() (*feishusdk.ResultStorage, error) {
		called++
		return &feishusdk.ResultStorage{}, nil
	}
	resultReporterFactory = func(storage *feishusdk.ResultStorage) (*resultReporter, error) {
...
func TestEnsureResultReporterStartsOnce(t *testing.T) {
	...
	os.Setenv(feishusdk.EnvResultBitableURL, "https://bitable")
	storageCalls := 0
	feishuStorageFactory = func() (*feishusdk.ResultStorage, error) {
		storageCalls++
		return &feishusdk.ResultStorage{}, nil
