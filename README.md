...
- **`cmd`**
  - CLI entrypoints built on `pkg/webhook` and `pkg/singleurl`:
    ...
    - `create-tasks`: create tasks from Feishu sheets/bitables (single-url/general-search/profile-search).
...
- **Sheet task creator**: `go run ./cmd create-tasks --source sheet --task-type single-url --source-url "$SOURCE_SHEET_URL" --task-url "$TASK_BITABLE_URL" --limit 2000` (add `--poll-interval 2m` for continuous polling)
- **General search tasks**: `go run ./cmd create-tasks --source bitable --task-type general-search --date 2025-12-01 --source-url "$DRAMA_BITABLE_URL" --task-url "$TASK_BITABLE_URL"`
- **Profile search tasks**: `go run ./cmd create-tasks --source bitable --task-type profile-search --date 2025-12-01 --source-url "$ACCOUNT_BITABLE_URL" --task-url "$TASK_BITABLE_URL"`
