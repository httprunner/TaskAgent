...
type fakeDramaTaskClient struct {
	...
	createCalls [][]feishu.TaskRecordInput
	createErr   error
}

func (f *fakeDramaTaskClient) FetchBitableRows(ctx context.Context, rawURL string, opts *feishu.TaskQueryOptions) ([]feishu.BitableRow, error) {
	return f.rows, nil
}

func (f *fakeDramaTaskClient) CreateTaskRecords(ctx context.Context, rawURL string, records []feishu.TaskRecordInput, override *feishu.TaskFields) ([]string, error) {
	if f.createErr != nil {
		return nil, f.createErr
	}
	copyChunk := append([]feishu.TaskRecordInput(nil), records...)
	...
	client := &fakeDramaTaskClient{
		rows: []feishu.BitableRow{
			{Fields: map[string]any{
				feishu.DefaultDramaFields.DramaName:   "DramaA",
				feishu.DefaultDramaFields.DramaID:     "B1",
				feishu.DefaultDramaFields.SearchAlias: "AliasA|AliasB|DramaA",
				feishu.DefaultDramaFields.CaptureDate: "2025-12-05",
			}},
			{Fields: map[string]any{
				feishu.DefaultDramaFields.DramaName:   "DramaB",
				feishu.DefaultDramaFields.DramaID:     "B2",
				feishu.DefaultDramaFields.SearchAlias: "AliasC",
				feishu.DefaultDramaFields.CaptureDate: fmt.Sprintf("%d", ts),
			}},
			{Fields: map[string]any{
				feishu.DefaultDramaFields.DramaName:   "DramaSkip",
				feishu.DefaultDramaFields.DramaID:     "B3",
				feishu.DefaultDramaFields.SearchAlias: "Other",
				feishu.DefaultDramaFields.CaptureDate: "2025-12-04",
			}},
		},
	}
...
func TestCreateDramaSearchTasksErrors(t *testing.T) {
	...
	cfg.BatchSize = 1
	client.rows = []feishu.BitableRow{
		{Fields: map[string]any{
			feishu.DefaultDramaFields.DramaName:   "DramaA",
			feishu.DefaultDramaFields.DramaID:     "B1",
			feishu.DefaultDramaFields.SearchAlias: "AliasA",
			feishu.DefaultDramaFields.CaptureDate: "2025-12-05",
		}},
	}
	if _, err := CreateDramaSearchTasks(context.Background(), cfg); err == nil {
