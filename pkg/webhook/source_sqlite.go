...
func newSQLiteSummarySource(fields summaryFieldConfig, opts Options) (summaryDataSource, error) {
	dbPath := strings.TrimSpace(opts.SQLitePath)
	if dbPath == "" {
		var err error
		dbPath, err = taskagent.ResolveResultDBPath()
		if err != nil {
			return nil, err
		}
	}
...
func pickTableEnv(key, fallback string) string {
	if val := taskagent.EnvString(key, ""); val != "" {
		return val
	}
	return fallback
}
