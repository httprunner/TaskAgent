...
func (w *WebhookResultWorker) processOnce(ctx context.Context) error {
	...
	for _, row := range rows {
		...
		key := fmt.Sprintf("%d|%s", row.ParentTaskID, strings.TrimSpace(row.GroupID))
		if w.shouldSkip(key) {
			continue
		}
...
func (w *WebhookResultWorker) handleRow(ctx context.Context, row webhookResultRow) error {
	...
	if len(records) == 0 {
		// Keep behavior aligned with legacy worker: no records -> treat as done to avoid infinite retries.
