...
func MirrorDramaRowsIfNeeded(tableURL string, rows []feishu.BitableRow) {
...
func isDramaCatalogURL(raw string) bool {
	envURL := strings.TrimSpace(os.Getenv("DRAMA_BITABLE_URL"))
	if envURL == "" || strings.TrimSpace(raw) == "" {
		return false
	}
	envRef, envErr := feishu.ParseBitableURL(envURL)
	rawRef, rawErr := feishu.ParseBitableURL(raw)
...
type dramaDetailStore struct {
	...
	stmt    *sql.Stmt
	table   string
	fields  feishu.DramaFields
	columns []string
}

func newDramaDetailStore() (*dramaDetailStore, error) {
	table := strings.TrimSpace(os.Getenv("DRAMA_SQLITE_TABLE"))
	if table == "" {
		table = "drama_catalog"
	}
...
func buildDramaColumnOrder(fields feishu.DramaFields) []string {
...
func ensureDramaSchema(db *sql.DB, table string, fields feishu.DramaFields, columns []string) error {
...
func pickDramaColumnType(name string, fields feishu.DramaFields) string {
	switch name {
...
func prepareDramaUpsert(db *sql.DB, table string, fields feishu.DramaFields, columns []string) (*sql.Stmt, error) {
...
func (s *dramaDetailStore) UpsertRows(rows []feishu.BitableRow) error {
	if s == nil || len(rows) == 0 {
		return nil
	}
...
func (s *dramaDetailStore) upsert(row feishu.BitableRow) error {
	if row.Fields == nil {
		return nil
	}
